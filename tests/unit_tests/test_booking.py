...
def test_insufficient_points_balance_should_return_error(client):
    ...
    expected_text = ("You have " + str(club_points) +
                     " points which is not enough to book " +
                     str(required_places) + " places.")
    assert expected_text in response.data.decode('UTF-8')


# if placesRequired > availablePlaces
def test_required_more_places_than_available_places_should_return_405(client):
    """
    ...
    assert response.status_code == 405
    assert ("You can&#39;t book more places than"
...
def test_book_negative_number_of_places_should_return_error(client):
    ...
    assert response.status_code == 405
    assert ("Booking a negative number of "
            "places or 0 is forbidden."
            in response.data.decode('UTF-8'))
...
def test_booked_places_should_subtract_club_points(client):
    ...
    assert ("Points available: " +
            str(club_points - places_required)
            in response.data.decode('UTF-8'))
...
def test_booking_more_than_max_booking_places_should_return_error(client):
    ...
    assert response.status_code == 405
    assert ("Booking more than " +
            str(MAX_BOOKING) + " places is forbidden."
            in response.data.decode('UTF-8'))
...
def test_book_past_competition_should_return_error(client):
    ...
    assert ("Booking a past competition is forbidden."
            in response.data.decode('UTF-8'))
